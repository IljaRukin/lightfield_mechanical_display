...
p1_lens = round(x_offset./p1(1));
x_offset = x_offset - p1(1)*p1_lens;
...
result(idx) = interp2(pic.T,x_pix(idx),y_pix(idx));
result = result/max(abs(result(:)));
imsave(['zebra_dv_',num2str(dv),'_d_',num2str(d),'_f_',num2str(f),'.png'],result);
